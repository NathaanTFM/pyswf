...
class DropShadowFilter(Filter):
    ...
    def __init__(self, color: RGBA, blurX: float, blurY: float, angle: float, distance: float, strength: float, innerShadow: bool, knockout: bool, compositeSource: bool, passes: int) -> None:
        self.color = color
        self.blurX = blurX
        self.blurY = blurY
        self.angle = angle
    ...
    def read(stream: SWFInputStream) -> Filter:
        ...
        return DropShadowFilter(color, blurX, blurY, angle, distance, strength, innerShadow, knockout, compositeSource, passes)
